...
class TestSlice10(unittest.TestCase):

    def test_rules(self):
        parsed_snippet = json.loads(file_get_contents(os.getcwd() + "/tests/slice10/slice10.json"))
        parsed_rules = json.loads(file_get_contents(os.getcwd() + "/tests/slice10/rules.json"))
        output = open(os.getcwd() + "/tests/slice10/slice10_rules.out", "r")
    ...
    def test_rules2(self):
        parsed_snippet = json.loads(file_get_contents(os.getcwd() + "/tests/slice10/slice10.json"))
        parsed_rules = json.loads(file_get_contents(os.getcwd() + "/tests/slice10/rules2.json"))
        output = open(os.getcwd() + "/tests/slice10/slice10_rules2.out", "r")
        vulnerabilities = Vulnerability.build_vulnerabilities(parsed_rules)
        s = Searcher(parsed_snippet['body'], vulnerabilities)
        self.assertEqual(getStr(s.output), output.read(), "Should be equal")
        output.close()
